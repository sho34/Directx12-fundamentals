...
//------------------------ FOR TRAINGLE RENDERING -------------------------// 
class simple_solid_pso : public pipeline_state_object
{
public:
	simple_solid_pso(ID3D12Device2* p_device, ID3D12RootSignature* p_root_signature)
		: pipeline_state_object(p_device, p_root_signature)
	{ }

private:
	ComPtr<ID3DBlob> m_vertex_shader{ load_shader(LOAD_SHADER(L"cube_vs.cso")) };
	ComPtr<ID3DBlob> m_pixel_shader{ load_shader(LOAD_SHADER(L"cube_ps.cso")) };
...
protected:
	...
	void configure_input_layout() override
	{
		m_input_element_descriptors = {
			...
			// per instance data.
			{"INSTANCE_TRANSFORM", 0, DXGI_FORMAT_R32G32B32_FLOAT, 1, 0,  D3D12_INPUT_CLASSIFICATION_PER_INSTANCE_DATA, 1},
			{"INSTANCE_TRANSFORM", 1, DXGI_FORMAT_R32G32B32_FLOAT, 1, 16, D3D12_INPUT_CLASSIFICATION_PER_INSTANCE_DATA, 1},
			{"INSTANCE_TRANSFORM", 2, DXGI_FORMAT_R32G32B32_FLOAT, 1, 32, D3D12_INPUT_CLASSIFICATION_PER_INSTANCE_DATA, 1},
			{"INSTANCE_TRANSFORM", 3, DXGI_FORMAT_R32G32B32_FLOAT, 1, 48, D3D12_INPUT_CLASSIFICATION_PER_INSTANCE_DATA, 1}
	...
	{
		m_pipeline_state_desc.PrimitiveTopologyType = D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE;
	}
...
//------------------------ FOR TRAINGLE RENDERING -------------------------// 
class simple_wireframe_pso : public pipeline_state_object
{
public:
	simple_wireframe_pso(ID3D12Device2* p_device, ID3D12RootSignature* p_root_signature)
		: pipeline_state_object(p_device, p_root_signature)
	{
	}
