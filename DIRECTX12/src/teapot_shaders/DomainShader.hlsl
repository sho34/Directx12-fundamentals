...
// from the CPU
struct ConstantBuffPerObj
{
    row_major float4x4 wvpMat;
};

ConstantBuffer<ConstantBuffPerObj> constPerObject : register(b0);
...
float3 evaluateBezier(
    const OutputPatch<HullToDomain, NUM_OF_CONTROL_POINTS> bezpatch, 
...
[domain("quad")]
DomainToPixel main(
    PatchConstantData input, float2 domain : SV_DomainLocation, 
    const OutputPatch<HullToDomain, NUM_OF_CONTROL_POINTS> patch, 
